...
const ProductShowcaseSection: React.FC = () => {
  ...
  return <div className="w-full px-4 mt-6 text-white bg-zinc-50">
      {/* Courses Section */}
      <div className="mb-8">
        <div onClick={() => toggleSection('courses')} className="flex items-center justify-between cursor-pointer p-4 rounded-lg mb-4 bg-yellow-500">
          <div className="flex items-center space-x-3">
            
            <h2 className="text-xl font-bold text-zinc-950">🎓 1000+ Courses List</h2>
          </div>
          {expandedSection === 'courses' ? <ChevronUp /> : <ChevronDown />}
        </div>
        
        {expandedSection === 'courses' && <div className="space-y-4">
            <p className="text-gray-300 mb-4">
              Unlock access to 1,000+ premium courses across all high-income niches. Here's a sneak peek:
            </p>
            {courseCategories.map((category, index) => <div key={index} className="bg-gray-900 p-4 rounded-lg">
                <h3 className="font-bold text-lg mb-3 text-yellow-400">{category.title}</h3>
                <div className="grid grid-cols-1 gap-2">
                  {category.courses.map((course, courseIndex) => <div key={courseIndex} className="flex items-center space-x-2">
      ...
      <div className="mb-8">
        <div onClick={() => toggleSection('products')} className="flex items-center justify-between cursor-pointer p-4 rounded-lg mb-4 bg-yellow-500">
          <div className="flex items-center space-x-3">
            
            <h2 className="text-xl font-bold text-zinc-950">📦 40 Million+ Digital Products List</h2>
          </div>
          {expandedSection === 'products' ? <ChevronUp /> : <ChevronDown />}
        </div>
        
        {expandedSection === 'products' && <div className="space-y-4">
            <p className="text-gray-300 mb-4">
              📦 40 Million+ Digital Products Included!
            </p>
            {digitalProductBundles.map((bundle, index) => <div key={index} className="bg-gray-900 p-4 rounded-lg">
                <h3 className="font-bold text-lg mb-3 text-blue-400">{bundle.title}</h3>
                <div className="grid grid-cols-1 gap-2">
                  {bundle.items.map((item, itemIndex) => <div key={itemIndex} className="flex items-center space-x-2">
                      <span className="text-green-400">✓</span>
      ...
      {/* Reels Section */}
      <div className="mb-8">
        <div onClick={() => toggleSection('reels')} className="flex items-center justify-between cursor-pointer p-4 rounded-lg mb-4 bg-yellow-500">
          <div className="flex items-center space-x-3">
            
            <h2 className="text-xl font-bold text-zinc-950">🎞️ 200K+ Reels List</h2>
          </div>
          {expandedSection === 'reels' ? <ChevronUp /> : <ChevronDown />}
        </div>
        
        {expandedSection === 'reels' && <div className="bg-gray-900 p-4 rounded-lg">
            <p className="text-gray-300 mb-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {reelsCategories.map((category, index) => <div key={index} className="flex items-center space-x-2">
                  <span className="text-green-400">✅</span>
