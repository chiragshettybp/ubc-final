  ...
  return (
    <motion.div 
      className={`relative w-full p-4 rounded-lg border transition-all duration-300 group cursor-pointer 
        ${active 
          ? 'bg-purple-lighter border-purple shadow-lg scale-[1.02]' 
          : 'bg-white border-gray-200 hover:border-purple/50 hover:shadow-md'}`} 
      onClick={onClick}
      whileTap={{ scale: 0.98 }}
    >
...
const ProductVariants: React.FC = () => {
  ...
  return (
    ...
    >
      <VariantOption 
        active={selectedVariant === 'one'} 
        variant="one" 
        onClick={() => setSelectedVariant('one')} 
      />
      <VariantOption 
        active={selectedVariant === 'two'} 
        variant="two" 
        onClick={() => setSelectedVariant('two')} 
      />

      <motion.div 
        className="w-full bg-purple rounded-b-lg p-3 text-white flex items-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="w-12 h-10 bg-gray-100 rounded-md mr-3 overflow-hidden">
          <img 
            src="/img/body-scrub-4-50x-png.png" 
            alt="Body Scrub" 
            className="w-full h-full object-cover"
          />
        </div>
      ...
      <motion.div 
        className="flex items-center mt-4 ml-4 text-orange"
