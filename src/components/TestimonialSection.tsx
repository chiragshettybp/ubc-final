...
const TestimonialCard: React.FC<{ testimonial: Testimonial; index: number }> = ({ testimonial, index }) => {
  return (
    <motion.div 
      className="bg-white rounded-lg p-5 shadow-md border border-gray-100 flex flex-col h-full"
    ...
    >
      <div className="flex items-center mb-4">
        ...
        {testimonial.verified && (
          <div className="ml-auto bg-purple-lighter px-2 py-1 rounded-full flex items-center">
      ...
      <div className="flex mb-3">
        {[...Array(5)].map((_, i) => (
          <Star 
            key={i} 
            className={`w-3.5 h-3.5 ${i < testimonial.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} 
          />
        ))}
      </div>
...
const TestimonialSection: React.FC = () => {
  return (
    <section className="w-full bg-purple-lighter/50 py-8">
